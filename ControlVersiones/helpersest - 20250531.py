...
def leer_estudiantes(date='nodate'):
    """
    Lee y concatena múltiples archivos Excel (.xlsx) con información de estudiantes.
    Devuelve un único DataFrame consolidado con datos limpios y normalizados.
    ...
    # Validación de existencia de archivos Excel
    if not excel_files:
    ...
    dataframes = []
    columnas_requeridas = {
        'Periodo', 'NRC', 'Lista_Cruzada', 'ID_Estudiante','Tipo_Documento', 'Documento', 'Correo_Principal_Estudiante',
        'Nombre_Estudiante', 'Apellidos_Estudiante', 'Tipo_Cancelación_Curso','Fecha_Actividad'
    }

    # Verificar si las columnas requeridas están presentes en los archivos
    for file in excel_files:
        ...
        #Normalización de nombres de columnas removiendo espacios y reemplazando por guiones bajos
        df.columns = [c.strip().replace(' ', '_') for c in df.columns]

        # Verificación de columnas requeridas
        if not columnas_requeridas.issubset(df.columns):
            faltantes = columnas_requeridas - set(df.columns)
            print(f"⚠️ Advertencia: El archivo {file} no contiene todas las columnas requeridas: {faltantes}")
            continue

        # Conversión y normalización de datos a tipos adecuados según lo requerido : string y titulos
        ...
        try:
            ...
            if date != 'nodate':
                df = df.loc[df['Fecha_Actividad'] >= pd.to_datetime(date)]
        except Exception as e:
