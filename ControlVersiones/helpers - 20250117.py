...
def leer_estudiantes(date='nodate'):
    ...
    for filename in os.listdir(directory):
        if filename.endswith(".xlsx"):
            ...
            # Manipulación de los datos. Reemplazar espacios con "_"
            ...
            data['ID_Estudiante']  = data['ID_Estudiante'].apply(lambda x: corregirId(x))
            #data['ID_Estudiante']  = data['ID_Estudiante'].zfill(9)
            data['Nombre_Estudiante'] = data.Nombre_Estudiante.str.title()
            data['Apellidos_Estudiante'] = data.Apellidos_Estudiante.str.title()
            #data = data[['Periodo', 'NRC', 'ID_Estudiante', 'Correo_Principal_Estudiante','Nombre_Estudiante', 'Apellidos_Estudiante', 'Tipo_Cancelación_Curso', 'Fecha_Actividad']] #Actualizacion 2024/04/10
            #data = data[['Periodo', 'NRC', 'ID_Estudiante', 'Tipo_Documento', 'Documento', 'Correo_Principal_Estudiante','Nombre_Estudiante', 'Apellidos_Estudiante', 'Tipo_Cancelación_Curso', 'Fecha_Actividad']] 2024/07/12
...
def corregirId(id):
    '''
    Función que recibe como entrada un ID.
    Devuelve el ID en el formato correcto.
    Añade los ceros correspondientes al principio del ID.
    '''
    cadena   = id
    longitud = len(cadena)

    # Añademos los ceros al ID
    for i in range(0, 9 - longitud):
        cadena = '0' + cadena
        
    return cadena

def crearArchivos(data, course_name, tform, tacc):
    '''
    ...
    #print("Tipo formacipon;"+tform)
    #print("Tipo accion"+tacc)
    #input()

    # Ciclo para recorrer el dataframe
    ...
    numberStudents = [course_name[-5:], line_count]
